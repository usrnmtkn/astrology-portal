...
## Amendment, 2026-08-13: one photographable clause does not pass a passage

```text
A passage does not pass because it contains one photographable clause. Every sentence must either advance the lived scene, state a specific consequence, or provide necessary astrology-to-life perspective. A generic astrology-summary sentence fails the passage even when another sentence passes the photograph test.
```

The photograph test is a minimum condition on the passage, not a verdict. It is evaluated per sentence for the summary gate and per passage for the observability gate. One lived sentence cannot launder two abstract ones after it.

Worked case: `jupiter|conjunction|south_node` contains observable material ("A teacher, trip, disappointment, or opportunity shows up", "the same belief is back on the table") and therefore satisfies the photograph minimum, yet fails the passage on "In this karmic framework, repetition carries a philosophical or spiritual lesson," which could appear unchanged in a generic astrology book.

## Amendment, 2026-08-13: concrete-noun density is a lint signal, not voice evidence

Counting observable nouns can reward a list. "The meeting, chair, email, coffee, deadline, phone, car, table" has eight and is not writing. What the benchmark demonstrates is causal scene density: the concrete material forms one coherent human sequence in which each item follows from the last (pull over an extra chair, order for the table, make the tense room laugh, spend freely, add another late night, body hits its limit). Density metrics may inform lint; they may never be cited as evidence that a passage sounds like the owner.

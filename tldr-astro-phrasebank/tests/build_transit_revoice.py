#!/usr/bin/env python3
"""
build_transit_revoice.py — -quality flowing paragraphs for transit/aspect DETAIL.
...
def main():
    ...
    for rid, r in idx.items():
        if rid in REVOICE:
            r["expanded_narrative"] = REVOICE[rid]; r["revoice_version"] = "-quality-v1-authored"; hand += 1
        else:
            para = compose(r)
            if para:
                r["expanded_narrative"] = para; r["revoice_version"] = "-quality-v1-composed"; composed += 1
            else:
                skipped += 1

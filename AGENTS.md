...
facts: Sky cards, article headers and Key dates, Calendar day/week/month and
season arcs, plus the shared You/Friends transit ranges and countdowns.
Use the selected location's time zone consistently for dates, exact times,
calendar keys, and remaining days, even when the browser uses another zone.
Never substitute fixed sign-season tables for calculated ingress timestamps.

For current direct placements, the primary card, article, and countdown must
describe the same continuous visit. A residency spanning retrograde return
visits belongs on a separate, explicitly labeled full-residency line. Preserve
the final verified exit in article body variables that describe the residency.
...
2026 is a regression case), retrograde motion, a return visit, and a single-pass
Sun season. Verify selected zones across midnight, year boundaries and DST,
then check the rendered dates after hydration, navigation and reload. Use
calculated boundaries and explicit fixture expectations, not matching strings
generated by the same formatter as proof of agreement. Run the focused date
and browser regressions from a fresh build and repeat the affected rendered
flow on the deployed main revision before claiming the discrepancy fixed.

Owner direction: [task dated 2026-09-14](thread:01a0a070-33be-7811-9a73-86df25435b3d).
See [date consistency QA](docs/qa/app-event-date-consistency.md) and
[placement visit QA](docs/qa/placement-visit-date-consistency.md).

...
assert.match(reportModelClient, /import \{ getVercelOidcToken \} from "@vercel\/oidc"/u, "Preview transport must retrieve Vercel OIDC at runtime instead of assuming a plain environment variable exists.");
assert.ok(packageLock.packages?.["node_modules/@vercel/oidc"], "The existing @vercel/functions dependency must install @vercel/oidc for runtime token retrieval.");
...
assert.match(reportModelClient, /await getVercelOidcToken\(\)/u, "Preview fallback must request the OIDC token from the Vercel request context at runtime.");
assert.doesNotMatch(reportModelClient, /process\.env\.VERCEL_OIDC_TOKEN/u, "Preview fallback must not depend on VERCEL_OIDC_TOKEN being injected as a plain environment variable.");
assert.match(reportModelClient, /AI_GATEWAY_API_KEY/u, "Preview fallback may still use an explicitly configured AI Gateway key when present.");

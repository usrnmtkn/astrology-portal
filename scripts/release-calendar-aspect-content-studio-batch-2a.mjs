...
const batchRel = "apps/web/src/content/fallbackArchitectureV3/authored-inputs/calendar-aspect-content-studio-batch-2a-v1.json";
const approvalRel = "packages/astro-knowledge/review/sky-calendar-composed-first-glance-2026-08-31/batch-2a-venus-saturn-squares-owner-approval.json";
const releaseRel = "packages/astro-knowledge/review/sky-calendar-composed-first-glance-2026-08-31/batch-2a-venus-saturn-squares-serving-release.json";
...
if (batch.batch_id !== release.batch_id || approval.batch_id !== release.batch_id) {
  throw new Error("Batch 2A release identity mismatch.");
}
if (approval.owner_approved !== true || approval.review_status !== "approved") {
  throw new Error("Batch 2A exact owner approval is missing.");
}
if (release.serving_enabled !== true || release.owner_authorized !== true) {
  throw new Error("Batch 2A serving release is not owner-authorized.");
}
if (release.approved_payload_sha256 !== approval.draft_payload_sha256) {
  throw new Error("Batch 2A serving release does not match the approved payload hash.");
}
if (!Array.isArray(batch.rows) || batch.rows.length !== 24 || release.expected_row_count !== 24) {
  throw new Error("Batch 2A release must contain exactly 24 rows.");
}
const batchKeys = batch.rows.map((row) => row.contentKey);
if (!sameArray(batchKeys, approval.approved_keys)) {
  throw new Error("Batch 2A approved key list does not match the release payload.");
}
...
for (const approved of batch.rows) {
  ...
  current.note = "sky aspect sign-specific; exact owner-approved 2026-09-01; serving release authorized 2026-09-01";
}

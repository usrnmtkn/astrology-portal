...
assert.deepEqual(replies.map(reply => reply.id), [1, 4, 2, 5, 6, 3]);
assert.equal(replies[3].ok, false);
assert.equal(replies[3].error, "fixture calculation failure");
assert.equal(replies[4].value, JSON.stringify({ name: "getLunarCalendarWeek", args: ["2027-01-12", "Asia/Tokyo"] }));
assert.equal(replies[5].ok, true);
console.log("PASS: Calendar work overtakes queued timing enrichment; inputs, request identity, serial execution and failure recovery are preserved.");

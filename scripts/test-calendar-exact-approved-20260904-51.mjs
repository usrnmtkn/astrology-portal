...
const previous = JSON.parse(fs.readFileSync(path.join(repoRoot, "packages/astro-knowledge/review/sky-calendar-exact-approved-2026-09-04-batch-30/current-owner-payloads.json"), "utf8"));
...
assert.equal(new Set(rows.map((row) => row.contentKey)).size, 51);
assert.equal(evidence.memberCount, 51);
assert.equal(manifest.rowCount, 51);
assert.equal(evidence.ownerStatement, "please proceed with next steps");
...
assert.deepEqual(changedTransitFiles, changed.sort(), "transit changes escaped the 51-row boundary");
...
console.log("Exact Calendar release verified: 51 approved rows changed, 164 prior payloads preserved, 2 protected benchmarks untouched.");

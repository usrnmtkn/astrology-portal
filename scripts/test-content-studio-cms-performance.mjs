...
assert.match(dashboard, /\(loadedRows, complete\) => \{[\s\S]{0,320}mergeContentInventory\(current, loadedRows\)/u, "Content Studio should paint inventory progressively instead of waiting for the entire table.");
assert.match(dashboard, /limit=\$\{pageSize\}&view=inventory/u, "Every CMS inventory page, including Friends Transits visibility=all, must use the compact projection so the dashboard can load.");

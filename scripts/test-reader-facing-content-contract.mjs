...
const adminBrowseComponents = read("apps/admin/src/AdminBrowseComponents.tsx");
assert.match(adminBrowseComponents, /aria-label=\{label\}/, "Shared filters must preserve their accessible group labels.");
const writingSurfaceSourceMap = read("apps/admin/src/writingSurfaceSourceMap.ts");
const generatedContent = read("apps/web/src/content/generatedContentEligibility.ts") + read("apps/web/src/services/generatedContent.ts");
...
assert.match(adminDashboard, /<AdminFilterBar[\s\S]*?label="Compatibility filters"/, "Compatibility admin surface must include dedicated filters.");

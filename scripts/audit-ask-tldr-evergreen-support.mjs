...
const uniqueSemanticAuthorityGaps = Object.values(rows
  .filter((row) => row.status === "primary_semantic_gap")
  .reduce((acc, row) => {
    const authorityKey = row.primary?.factorKey ?? row.primary?.id ?? `unknown:${row.questionId}`;
    if (!acc[authorityKey]) {
      acc[authorityKey] = {
        factorKey: row.primary?.factorKey ?? null,
        primaryId: row.primary?.id ?? null,
        kind: row.primary?.kind ?? null,
        governedCanonicalIds: row.primary?.governedCanonicalIds ?? [],
        questionIds: []
      };
    }
    acc[authorityKey].questionIds.push(row.questionId);
    return acc;
  }, {}))
  .map((gap) => ({
    ...gap,
...
console.log(JSON.stringify({
  schema: "ask-tldr-evergreen-support-audit.v4",

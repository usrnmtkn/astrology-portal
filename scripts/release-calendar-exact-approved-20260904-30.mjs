...
const previousPayloadRelative = "packages/astro-knowledge/review/sky-calendar-exact-approved-2026-09-03/current-owner-payloads.json";
...
if (ruling.batchId !== "sky-calendar-exact-approved-2026-09-04-batch-30") throw new Error("Unexpected batch.");
...
const rows = ruling.payloadFiles.flatMap((file) => {
  ...
  if (`${payload.family}.json` !== file || payload.rowCount !== payload.rows?.length) throw new Error(`${file}: invalid payload.`);
  return payload.rows;
}).sort((a, b) => a.contentKey.localeCompare(b.contentKey));

if (rows.length !== 30 || ruling.memberCount !== 30 || new Set(rows.map((r) => r.contentKey)).size !== 30) {
  throw new Error("Expected exactly 30 unique approved rows.");
}
...
fs.writeFileSync(path.join(reviewRoot, "shipping-manifest.json"), `${JSON.stringify({
  schemaVersion: 2,
  name: "Sky Calendar exact owner-approved release, 2026-09-04 batch 30",
  batchId: ruling.batchId,
  approvedAt: ruling.approvedAt,
  ownerStatement: ruling.ownerStatement,
  surface: ruling.surface,
...
fs.writeFileSync(path.join(reviewRoot, "current-owner-payloads.json"), `${JSON.stringify({
  schemaVersion: 2,
  name: "Current owner-approved Sky Calendar exact aspect payloads after 2026-09-04 batch 30",
  rowCount: 215,
  approvedOverlayCount: 30,
...
console.log("Released 30 hash-bound owner-approved exact Calendar aspect rows.");

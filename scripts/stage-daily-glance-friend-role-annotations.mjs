...
const applyApproved = process.argv.includes("--apply-owner-approved");
const sha256 = (value) => crypto.createHash("sha256").update(value).digest("hex");

if (!applyApproved) {
  throw new Error("Refusing to write without --apply-owner-approved.");
}
...
for (const entry of review.records) {
  assert.equal(entry.status, "OWNER_APPROVED");
...
console.log(`Applied ${changedFields} owner-approved body_they fields across ${review.records.length} Daily Glance keys.`);

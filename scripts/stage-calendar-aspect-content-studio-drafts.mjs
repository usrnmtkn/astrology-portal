...
const phrasebookPath = path.join(
  repoRoot,
...
if (!Array.isArray(stage.drafts) || stage.drafts.length !== 48) {
  throw new Error(`Expected 48 Calendar aspect drafts; found ${stage.drafts?.length ?? 0}.`);
}
...
if (composedDrafts.length !== 24 || signSpecificDrafts.length !== 24) {
  throw new Error(`Expected 24 composed + 24 sign-specific drafts; found ${composedDrafts.length} + ${signSpecificDrafts.length}.`);
}
...
const rows = stage.drafts.map((draft) => {
  ...
  return {
    ...
    facts: {
      ...
      readerServing: false,
      stageOnly: true,
      sourceSchemaVersion: SKY_V4_CONTENT_STUDIO_PACKAGE_VERSION,
      calendarAspectDraftPackage: CALENDAR_ASPECT_DRAFT_PACKAGE_VERSION,
      calendarAspectSourceKind: draft.sourceKind
    },
    knowledge_ids: [],
    source_snapshot: {
      sourcePackage: SKY_V4_CONTENT_STUDIO_PACKAGE_VERSION,
...
async function verifyRows() {
  ...
  if (!Array.isArray(payload) || payload.length !== 48) {
    throw new Error(`Expected 48 staged Content Studio rows; found ${Array.isArray(payload) ? payload.length : 0}.`);
  }

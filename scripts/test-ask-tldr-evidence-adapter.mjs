...
assert.equal(
  future.some((item) => item.kind === "solar_return_overlay" && item.points.some((point) => ["Chiron", "Lilith", "Black Moon Lilith", "North Node", "South Node"].includes(point))),
  false,
  "Ask TLDR must honor the owner-approved SR-overlay eligibility boundary: Sun through Pluto only; Chiron, Lilith, and the Nodes are excluded."
);

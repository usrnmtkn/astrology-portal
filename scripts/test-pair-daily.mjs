...
assert.match(
  first.body,
  /^@project-author, you are letting a slow reply be a slow reply instead of assuming something is wrong, while @matthew_mezo is fixing the thing at home that makes the day harder every single time\./u
);
...
assert.match(second.body, /^@project-author, your attention is on/u);
...
assert.doesNotMatch(readerHandleFallback.body, /Marie|@project-author/u);

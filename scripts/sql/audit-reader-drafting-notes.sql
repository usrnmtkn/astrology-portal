...
and v#>>'{}' ~* '(^ *[a-z][a-z0-9_]* resolves as (interpretive|fact|gap|calculated|structural)|^ *SOURCE_GAP: *[a-z0-9_]+ has no authored source|^ *(REVIEWED|DRAFT|APPROVED|CONFIRMED|SOURCE_ONLY) *[·|]|Engine note:|layer two evergreen|needs_review|Templated article|Bespoke edition|Your published blocks, aspect threads moved to slots|Twelve rising blocks, authored per edition|Block architecture per|No axis paragraph on this surface, per the ruling|On approval: *imports|pending engine confirmation|\{\{ *[A-Za-z][A-Za-z0-9]* *:|\{\{ *[A-Za-z][A-Za-z0-9_]*[[:space:]]+[^[:space:]{}][^{}]*\}\}|^ *(Here is|Here.s) (your|the) (revised |updated |requested )?(draft|article|copy|passage))'

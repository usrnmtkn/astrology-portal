...
function verify(selected: Record<string, string>) {
  ...
  if (result.copy.visible) {
    ...
    for (const key of result.copy.selectedPlacementKeys) {
      for (const name of ["livedExperienceClause", "situationPhrase", "responseClause"] as const)
...
// Identical text in separate sources must remain independently editable.
...
// A different valid template order maps the actual source text, not hardcoded sentences.
...
assert.ok(one.paragraphs[1].some(part => part.sourceKey === "cms/sky-debility/signConditionOne"));
...
for (const missing of [null, ""]) {
  const gap = buildSkyDebilityComposition(original, edits({ [venusKey]: missing }));
...
console.log(JSON.stringify({ result: "passed", mappedCombinationStates: checked, parity: "byte-for-byte with reader assembler", exactSourceRouting: true, singleReadSnapshot: true }, null, 2));

...
const previousPayloadRelative = "packages/astro-knowledge/review/sky-calendar-exact-approved-2026-09-04-held-trines-33/current-owner-payloads.json";
...
if (rows.length !== 48 || ruling.memberCount !== 48 || new Set(rows.map((row) => row.contentKey)).size !== 48) {
  throw new Error("Expected exactly 48 unique North Node rows.");
}
for (const row of rows) {
  const parts = row.contentKey.split(".");
  if (parts.length !== 5 || parts[0] !== "sky" || parts[1] !== "aspect" || parts[4] !== "north-node") {
    throw new Error(`${row.contentKey}: outside North Node release scope.`);
  }
  if (!allowedAspects.has(parts[3]) || row.contentKey.includes("south-node") || row.contentKey.includes(".trine.")) {
    throw new Error(`${row.contentKey}: excluded aspect/node escaped release scope.`);
  }
...
for (const row of rows) {
  ...
  manifestRows.push({
    ...
    sourceCreatedByRelease: true,
    recordPath: recordRelative,
    bodySha256: row.bodySha256,
    previousReaderCopySha256: sha256(JSON.stringify({ summary: null, body: null, approvedVia: null })),
  });
}

fs.writeFileSync(path.join(reviewRoot, "shipping-manifest.json"), `${JSON.stringify({
  schemaVersion: 2,
  name: "Sky Calendar exact owner-approved North Node release, 2026-09-05, 48 rows",
  batchId: ruling.batchId,
  approvedAt: ruling.approvedAt,
  ownerStatement: ruling.ownerStatement,
  surface: ruling.surface,
  ...
  overlayPath: `${reviewRelative}/owner-payload-overlay.json`,
  rows: manifestRows,
}, null, 2)}\n`);
...
for (const row of rows) {
  ...
  overlayPayloads[legacyKey] = { sha256: sha256(JSON.stringify(payload)), payload };
}
fs.writeFileSync(path.join(reviewRoot, "owner-payload-overlay.json"), `${JSON.stringify({
  schemaVersion: 2,
  name: "North Node 48 exact-owner-approved overlay",
  batchId: ruling.batchId,
  previousProjectionPath: previousPayloadRelative,
  previousRowCount: 248,
  overlayRowCount: 48,
  combinedRowCount: 296,
  memberSetSha256: ruling.memberSetSha256,
  payloads: overlayPayloads,
}, null, 2)}\n`);

const previous = JSON.parse(fs.readFileSync(path.join(repoRoot, previousPayloadRelative), "utf8"));
if (previous.rowCount !== 248 || Object.keys(previous.payloads ?? {}).length !== 248) {
  throw new Error("Previous owner payload projection must contain exactly 248 rows.");
}
const currentPayloads = JSON.parse(JSON.stringify(previous.payloads));
for (const row of rows) {
  const legacyKey = row.contentKey.replace(/^sky\.aspect\./u, "sky.");
  if (Object.hasOwn(currentPayloads, legacyKey)) throw new Error(`${row.contentKey}: overlay collides with prior approved corpus.`);
  const payload = { summary: row.summary, body: row.body };
...
fs.writeFileSync(path.join(reviewRoot, "current-owner-payloads.json"), `${JSON.stringify({
  schemaVersion: 2,
  name: "Current owner-approved Sky Calendar exact aspect payloads after 2026-09-05 North Node 48",
  rowCount: 296,
  approvedOverlayCount: 48,
...
console.log("Released 48 hash-bound owner-approved North Node Calendar aspects.", {
  previousExactCount: 248,
  currentExactCount: 296,

...
globalThis.fetch = async (input, init = {}) => {
 ...
 if (url.pathname === "/rest/v1/content_publications") return Response.json(fixturePublications(stored).map(row => retired.has(row.content_key) ? { ...row, state: 'retired' } : row));

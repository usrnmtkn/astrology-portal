...
const unsupportedProfectionCandidate = {
  ...profectionCandidate,
  id: "profection:annual:11-voice-fixture",
  factorKey: "profection:annual:11-voice-fixture",
  houses: [11],
  facts: { ...profectionCandidate.facts, house: 11 }
};
const unsupportedProfection = resolveAskTldrGovernedFactor({
  ...unsupportedProfectionCandidate,
  score: 100,
  role: "primary",
  reasons: ["fixture"]
});
assert.equal(unsupportedProfection.governedMeaning.status, "partial");
const blocked = buildAskTldrVoiceEvidenceReceipt({
  question: governed.question,
  evidence: [unsupportedProfection],

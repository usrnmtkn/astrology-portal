...
assert.equal(taxonomy.status, "owner-approved");
assert.equal(taxonomy.ownerApproved, true);
...
console.log(`Ask TLDR taxonomy contract passed: ${taxonomy.pillarCount} pillars, ${questions.length} unique evergreen questions; owner approval is recorded while promotion and runtime remain closed.`);

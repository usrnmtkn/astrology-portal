...
assert.ok(pointRows.every((row) => row.generationAllowed === false), "Every Batch 1 point/angle row must fail closed.");
assert.ok(packets.rows.filter((row) => !pointRows.includes(row)).every((row) => row.generationAllowed === true), "Every Batch 1 non-point row must have a compliant packet.");

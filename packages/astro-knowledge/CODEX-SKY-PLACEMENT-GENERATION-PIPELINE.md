...
## Article structure (owner's  models, 2026-07-27)

The owner supplied two  pages as STRUCTURAL models (Sun-in-Leo horoscopes,
...
3. HOOK - doubles as 's "Takeaway" block (approved beat)
...
6. MOVES - 2-3 concrete ways to work with the transit (generated slot, new;
   's "7 ways" cut to the best 2-3, each held to the swap test)
7. Dated sky events during the transit (lunations/eclipses/aspects) in their
   own blocks below, COMPUTED only

's "last time this happened" history section is deliberately EXCLUDED:

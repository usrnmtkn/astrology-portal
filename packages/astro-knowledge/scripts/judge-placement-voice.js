...
// body?), does the register match the planet's tier. 1-3 verdict, same gates
// as the aspect pipeline: 3 auto-publish, 2 human-review, 1 regenerate.
//
...
function buildJudgePrompt(article, { tier = "", planet = "", sign = "" } = {}) {
  ...
  return [
    ...
    ``,
    `You are scoring a sky placement article${placementLabel}. Slots: an optional TAGLINE (2-5 word imperative under the title), HOOK, LIVED, TURN, and optional MOVES (2-3 concrete ways to work with the transit), rendered top to bottom as one article. The date range and dated sky events are computed by the app; they are not part of what you score.`,
    `The voice: ${spec.voiceDescription}`,
    tier ? `This article's register is ${TIER_HINT[tier] || tier}` : ``,
    ``,
    `Score the article 1-3:`,
    `  3 = in voice. The hook is a line someone would send to a friend; the lived beat shows the placement in ordinary behavior with its pace; the turn names where the gift becomes the problem and ends on the line with the most bite; the whole thing could ONLY be this planet in this sign.`,
    ...
    `  - Hook quality: tension, recognition, or a sharp claim - not an announcement ("X enters Y") and not a definition.`,

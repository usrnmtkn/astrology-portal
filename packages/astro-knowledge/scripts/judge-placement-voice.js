...
function buildJudgePrompt(article, { tier = "", planet = "", sign = "" } = {}) {
  ...
  return [
    ...
    `  - CURRENT SKY IS COLLECTIVE: new copy may not use "you", "your", or "part of you". Second person belongs to transit-to-natal copy. Older owner-approved gold examples are preserved verbatim, but their second-person phrasing is not reusable precedent.`,
    `  - LITERAL ENGLISH: read each sentence literally. An abstract noun may not perform an unnatural human action, and a metaphor may not require decoding. Conventional astrology language such as "Mars governs action" is allowed. If dressed-up phrasing replaces the actual behavior or consequence, score no higher than 2.`,
    ...
    `  - NAME THE COST: the turn must identify the actual consequence and stop at the strongest plain sentence. A dramatic metaphor, manufactured aphorism, or second conclusion caps the article at 2.`,
    ...
    `  - Flag any sentence matching the CC/SD constructions in voice/banned-constructions.json; the tic list identifies copy that reads as  or Spirit Daughter rather than the house voice. Owner-verbatim text is exempt. Treat nearby cases as a judged consideration, not an automatic fail: specific, falsifiable permission to do a nameable thing is house voice; generic affirmational permission is not.`,
    ``,

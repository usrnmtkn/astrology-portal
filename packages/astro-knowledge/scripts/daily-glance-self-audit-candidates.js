...
function deterministicLintRules(packet, config) {
  const fixed = [
    ...
    { id: "SOL-DIRECTIVE-body", rule: "Body contains 50–90 words in 3–5 sentences, uses second person, and does not repeat the complete headline." },
...
function renderSelfAuditWriterInput(packet, config, examples) {
  ...
  const rules = deterministicLintRules(packet, config).map((entry) => `- ${entry.id}: ${entry.rule}`).join("\n");
  let modelInput = directive
    ...
    .replace("- `{{LINT_RULES}}` — the deterministic lint spec, verbatim. These are hard gates: a candidate that fails any rule is discarded unread, regardless of quality.", `### Deterministic lint rules (hard gates; failures are discarded unread)\n${rules}`);
...
function selfAuditPacketLint(packet, modelInput, config, examples, currentPair = null) {
  ...
  const checks = [
    ...
    { id: "deterministic-lint-spec", passed: deterministicLintRules(packet, config).every((entry) => modelInput.includes(`- ${entry.id}: ${entry.rule}`)), details: `${deterministicLintRules(packet, config).length} exact lint rules included.` },
...
function lintSelfAuditCandidate(candidate, key, config) {
  ...
  const checks = [
    ...
    { id: "SOL-DIRECTIVE-body", passed: wordCount(candidate.body) >= 50 && wordCount(candidate.body) <= 90 && bodySentences.length >= 3 && bodySentences.length <= 5 && /\byou(?:r|rs|rself)?\b/iu.test(candidate.body), details: `${wordCount(candidate.body)} words; ${bodySentences.length} sentences` },

...
const sourcePath = path.join(editorialRoot, "owner-author-editorial-decisions.yaml");
...
function baseArtifact(kind, source, sourceSha256) {
  return {
    schemaVersion: 1,
    artifact: kind,
    generatedBy: "scripts/compile-owner-author-editorial-decisions.js",
...
function main() {
  ...
  console.log(`${verb} Project Author editorial policy (${result.compiled.active.length} active, ${result.compiled.unresolved.length} unresolved, ${result.compiled.artifacts.regression.activeCaseCount} active regressions).`);
}

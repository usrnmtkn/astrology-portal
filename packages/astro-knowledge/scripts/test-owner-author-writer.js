...
const skillRoot = path.join(repoRoot, ".agents", "skills", "owner-author-writer");
...
function main() {
  ...
  const wp1Overlay = JSON.parse(fs.readFileSync(path.join(packageRoot, "voice", "tldr-astro", "owner-author-writer", "ll-matrix-v13", "wp1-owner-approved-locked.json"), "utf8"));
  ...
  const ownerFeedbackAudit = fs.readFileSync(path.join(packageRoot, "voice", "tldr-astro", "owner-author-owner-feedback-audit.md"), "utf8");
  ...
  const taxonomy = require(path.join("..", "voice", "tldr-astro", "owner-author-writer", "failure-tags.json"));
  const requiredTags = [
  ...
  const invalidatedPool = require(path.join("..", "voice", "tldr-astro", "owner-author-writer", "surface-qualified-positive-exemplars.json"));
  ...
  const pool = require(path.join("..", "voice", "tldr-astro", "owner-author-writer", "surface-qualified-positive-exemplars-v2.json"));
  ...
  const formatDataset = require(path.join("..", "voice", "tldr-astro", "owner-author-writer", "sky-placement-format-exemplars-v4.json"));
  ...
  assert.match(packet.writerPrompt, /attached owner-authored Project Author passages/);
  ...
  assert.strictEqual(nodeAxisPacket.ownerCorpusWarmthEvidence.primary.sourceId, require(path.join("..", "voice", "tldr-astro", "owner-author-writer", "owner-corpus-warmth-foundations-v1.json")).records.find((entry) => entry.id === "warmth-foundation-jupiter-leo-v1").primary.sourceId);
  ...
  const writerPolicy = require(path.join("..", "config", "owner-author-writer-policy-v1.json"));
  assert.match(writerPolicy.voiceTarget.permanentRule, /-adjacent cadence is acceptable/);
  ...
  assert.match(judgePrompt, /\[CF-016\].*-adjacent warmth/);
  ...
  assert.match(agents, /\.agents\/skills\/owner-author-writer\/SKILL\.md/);
  const skill = fs.readFileSync(path.join(skillRoot, "SKILL.md"), "utf8");
  assert.doesNotMatch(skill, /\[TODO/);
  assert.match(skill, /name: owner-author-writer/);
  assert.match(skill, /Terra only at the end/);
  assert.match(skill, / can influence the softness of the delivery; Marie determines what the article notices/);
  ...
  console.log(`Project Author writer environment passed: ${index.entries.length} indexed excerpts, governed retrieval, authorship gate, feedback safety, and separated writer/judge roles.`);
}

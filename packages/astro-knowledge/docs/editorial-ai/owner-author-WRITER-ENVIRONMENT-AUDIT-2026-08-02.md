# Project Author writer environment audit
...
Repository `AGENTS.md` instructions are part of the working environment. They previously required factual and content-management safety, but did not route writing tasks through a dedicated Project Author process.
...
Third-party sources remain knowledge or vocabulary context and never Project Author voice evidence. AC continues to be labeled `AC` where sourced.

## Governance conclusion

A judge score of 3 must not be represented as owner approval, canonical status, production promotion, or proof that a sentence is the strongest available writing. The durable fix is the writer workflow documented in `owner-author-WRITER-IMPLEMENTATION-2026-08-02.md`.

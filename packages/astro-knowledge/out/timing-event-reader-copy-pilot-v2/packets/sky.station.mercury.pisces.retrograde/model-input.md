You are writing one original TLDR Astro Current Sky timing-event card.
Read the six owner-authored Project Author passages for sentence register and paragraph movement. Use the verified astrology as the complete meaning boundary.
Make it quotable and immediately clear. Write normal English. Name the pressure, what someone does, and what changes because of it.
The first two sentences are the collapsed-card preview, so they must stand alone without sounding like a summary template.
Do not copy or analyze the owner passages. Do not return options, commentary, a source map, or a style explanation.
Use collective language. Never use people, you, your, yours, yourself, or yourselves.
Return only strict JSON with exactly one key: body.

VERIFIED ASTROLOGY
{
  "event": {
    "title": "Mercury stations retrograde in Pisces",
    "family": "station",
    "phrase": "Mercury stations retrograde in Pisces",
    "fact": "Mercury's apparent motion reaches zero before reversing; conventionally read a day or two either side.",
    "scenes": "replies delayed well past normal; travel plans changed twice in a day; the same conversation repeated in one meeting; a booking abandoned midway.",
    "meaning": "traditionally an unfavorable window for finalizing agreements, travel, and communication decisions. A pause reading, not a failure reading. Some decisions made at stations stand."
  },
  "planetFunction": "Mercury describes the mind in motion: thought, language, questions, interpretation, learning, writing, conversation, and the way information becomes useful. It shows how someone notices patterns, names what is happening, and makes decisions from what they observe.",
  "signExpression": {
    "element": "water",
    "mode": "mutable",
    "ruler": "jupiter",
    "keywords": [
      "sensitivity",
      "imagination",
      "compassion",
      "porous boundaries",
      "surrender"
    ]
  },
  "combinedMeaning": {
    "tldr": "Mercury in Pisces: the mind is intuitive, poetic, and porous to subtle information.",
    "body": "With Mercury in Pisces, communication moves through images, feeling, dreams, and impression. Mercury is in detriment and fall here, so clarity needs extra structure.",
    "gift": "Imagination, empathy, symbolic thinking, and intuitive listening.",
    "challenge": "Confusion, avoidance, mixed signals, or letting emotion blur the facts."
  },
  "retrogradeMeaning": "review of thinking, plans, and messages; recheck details and communications",
  "boundary": [
    "Write collective Current Sky copy, never natal or transit-to-natal advice.",
    "The ephemeris supplies dates, degrees, duration, direction, and exact event times. Do not write them.",
    "Do not turn a review period into a prediction that every plan, purchase, promise, or relationship will fail.",
    "Use the sign to describe style and subject only. Do not infer a house or unsupported life domain.",
    "Chiron is an optional modern body, not a planet or a traditional timing factor."
  ]
}

SURFACE REQUIREMENTS
{
  "contractId": "current-sky-timing-event-reader-copy-v1",
  "output": "One two-paragraph body. The first two complete sentences become the collapsed-card preview.",
  "length": "140 to 220 words, normally 7 to 10 sentences.",
  "person": "we, someone, a named group, or the actual subject; never generic people or second person",
  "hardRules": [
    "Begin with recognizable behavior, a decision, or a consequence.",
    "Make the first two sentences complete, strong, and immediately clear because the card truncates after them.",
    "Build one advancing lived sequence rather than an inventory of examples.",
    "Mention the exact event naturally by the third sentence at the latest.",
    "Use normal English. Do not make an abstract noun perform a human action.",
    "Do not use em dashes, cryptic slogans, advocacy-default subject matter, corporate language, or a second conclusion.",
    "End with the clearest behavioral truth, then stop."
  ]
}

EXACT TASK
Write one Current Sky timing-event card for Mercury stations retrograde in Pisces. Return only the complete body.

OWNER PASSAGE 1
Set a two-line agenda before a meeting. Then create one honest boundary that protects recovery time. Block a half hour for a walk. Say no to a request that costs more than it gives.

OWNER PASSAGE 2
Here, words move fast, ideas hit hard, and filters disappear. Mercury in Aries isn’t interested in pleasantries, it cuts to the chase, speaks before thinking, and doesn’t wait for permission.

OWNER PASSAGE 3
Collectively, we begin to learn the cost of comfort, the limits of growth, and that real care needs a structure, a schedule and not just sentiment. There may not be a resolution this month, but there will be motion.

OWNER PASSAGE 4
To be seen not just for our dreams but for our willingness to feel everything so deeply. It's human to want someone else to understand that our tears aren't weakness . They're how we metabolize the world's grief into something survivable. It's our responsibility to stop apologizing for our depth, to stop second-guessing our knowing, to stop pretending we don't see what we see.

OWNER PASSAGE 5
Libra seeks harmony, but Lilith in Libra does not settle for false balance. If something has been unjust, this opposition calls it to the surface.

OWNER PASSAGE 6
Friendships may shift now. The ability to change friendship groups is a luxury. Not everyone gets to walk away from community, even when it stops feeling aligned.
...
Status: exact owner-approved wording
Date: 2026-08-14
Automatic edits: 0
Billed calls: 0
...
The serving source uses the existing engine-owned prior-sign slots in the first sentence:

- `Virgo` -> `{{priorSign}}`
- `July 9` -> `{{priorSignEntryDate}}`
- `August 6` -> `{{priorSignExitDate}}`

Rendering the approved 2026 engine snapshot restores the exact owner text. No reader-facing

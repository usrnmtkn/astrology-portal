...
- Focused real-product browser suite: PASS (`9/9`): ordinary placement, contextual overlay + retrograde, governed placement aspects, New Moon, Full Moon, exact eclipse, sign-aware eclipse fallback, current Node axis/North/South modules + Lilith, seasonal context, and exact-day Lilith station. GitHub Visual smoke exposed two aspect-heading integration contracts in sequence: the screen-reader-only `Aspect details` hierarchy and the visible continuous-placement label `Aspects shaping this transit`. Both were restored without changing reader copy; the two exact failed cases and the New Moon, Full Moon, exact-eclipse, and sign-aware-fallback cases pass together locally (`6/6`).
- Complete client-facing browser suite under the GitHub CI environment: PASS (`81/81`). A slow GitHub runner additionally exposed the existing Calendar Month-view remount exceeding Playwright's default five-second assertion window; the assertion now uses the suite's existing 15-second route budget and passes three concurrent repetitions (`3/3`). This is test synchronization only; no Calendar application behavior or reader copy changed.

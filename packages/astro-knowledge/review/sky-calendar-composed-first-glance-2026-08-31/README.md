...
Scope: the 24 currently serving sign-specific composed Calendar aspect forecasts in `packages/astro-knowledge/data/sky-calendar/composed-cards-v1.json`.
...
Do not copy these forecasts into the serving composed-card file until exact wording is owner-approved. After approval, update the canonical forecasts, refresh approval provenance and payload hashes as required by the package validation contract, rebuild the package, and run Calendar routing/regression checks.

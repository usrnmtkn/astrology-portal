# Sky Placement fallback deep audit
...
| Legacy thin standalone | 10 | Complete through owner review |
| SOURCE_GAP | 78 | Correctly hidden until approved article exists |

**Complete today:** 80 of 168 pages have governed full copy. **Editorial completion queue:** 88 pages (10 visible-but-thin, 78 fail closed).
...
84 historical four-slot candidates were audited with explicit planet/sign and reviewed cycle facts. 84 fail current policy; none were rewritten or promoted. Main failure counts: ED-029 373, ED-030 120, CF-006 84, untraced-duration 17, CF-002 6, CF-018 1.

The earlier semantic hook audit is not a rewrite mandate: it used now-superseded register rules and the model prose judge is advisory-only. This report relies on runtime selection, source status, deterministic policy, byte hashes, and owner approval records.

## Files

- `inventory.json`: all 168 pages, render lane, content key, mechanical checks, and recovery findings.
- `editorial-queue.md`: the exact 88-page completion queue.

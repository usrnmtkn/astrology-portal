...
export default function SkySectionPacketEditor({ section, disabled, facts, onChange }: {
  ...
  return <div role="group" aria-label="Section packet">
    <label className="admin-field-wide"><span>Section role</span>
      <AdminSelect aria-label="Section role" disabled={disabled || isPractical} value={section.role ?? "main"} onChange={event => onChange({ role: event.target.value })}>
    ...
    <label className="admin-field-wide"><span>Target depth</span>
      <AdminSelect aria-label="Target depth" disabled={disabled} value={section.depth ?? "standard"} onChange={event => onChange({ depth: event.target.value })}>
    ...
    {units.map((unit, index) => {
      ...
      return <details className="admin-workspace-details" key={unit.id}>
        <AdminDisclosureSummary>{name} {index + 1}{"job" in unit && unit.job ? ` · ${unit.job}` : ""}</AdminDisclosureSummary>
        {"job" in unit ? <label className="admin-field-wide"><span>Paragraph job</span>
          <StudioInput aria-label={`Paragraph job ${index + 1}`} value={unit.job} maxLength={120} disabled={disabled} onChange={event => update(unit.id, { job: event.target.value })} />
        </label> : <label className="admin-review-copy-editor"><span>Action</span>
          <StudioTextarea aria-label={`Practical action ${index + 1}`} className="admin-copy-field-summary" value={unit.action} maxLength={Math.max(0, 20000 - count + unit.action.length)} disabled={disabled} onChange={event => update(unit.id, { action: event.target.value })} />
        ...
        <div className="admin-sky-writing-source-actions" role="group" aria-label={`Arrange ${name.toLowerCase()} ${index + 1}`}>
          <StudioButton type="button" disabled={disabled || index === 0} onClick={() => move(index, -1)}>Move {name.toLowerCase()} {index + 1} up</StudioButton>
          <StudioButton type="button" disabled={disabled || index === units.length - 1} onClick={() => move(index, 1)}>Move {name.toLowerCase()} {index + 1} down</StudioButton>
          ...
          }}>Remove {name.toLowerCase()} {index + 1}</StudioButton>
        </div>
      </details>;
    })}
    <StudioButton type="button" disabled={disabled || units.length >= 24} onClick={() => setUnits([...units, isPractical
      ? { id: `item-${crypto.randomUUID()}`, action: "", phrases: [] }
      : { id: `paragraph-${crypto.randomUUID()}`, job: "", phrases: [] }])}>Add {isPractical ? "practical item" : "paragraph"}</StudioButton>
  </div>;
}

-- Make friend discovery forgiving when someone enters a longer or shorter
-- version of a name token (for example, "Prezner" for a profile named "Prez").
-- Exact and contiguous matches continue to rank first.
...
begin
  ...
  where profile.handle is not null
    ...
    and (
      ...
      or not exists (
        select 1
        from regexp_split_to_table(normalized_name, '\s+') as query_token
        where not exists (
          select 1
...
comment on function public.search_social_profiles(text) is
  'Returns up to 20 discoverable profiles matching normalized name substrings or token prefixes without exposing natal or contact data.';

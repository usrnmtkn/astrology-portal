-- Assign every signed-in member a readable, unique default handle.
--
-- The existing case-insensitive unique index remains the final authority.
-- This RPC retries with numeric suffixes so two people with the same display
-- name still receive different handles under concurrent profile creation.
...
comment on function public.ensure_own_social_profile(text, text, jsonb) is
  'Creates or synchronizes the caller social profile and atomically assigns a unique default handle when missing.';

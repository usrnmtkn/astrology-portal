...
function bundledSource(): CorrectionSource {
  return {
    ...(correctionManifest as CorrectionManifest),
...
function releasedPackage(source: CorrectionSource) {
  ...
  const expectedKeys = new Set(
    continuousPlanets.flatMap((planet) => signs.map((sign) => `sky-placement/article/${planet}/${sign}`))
  );
...
export function applySkyV4ContinuousCorpusCorrection(
  corpus: unknown,
  source: CorrectionSource = bundledSource()
) {

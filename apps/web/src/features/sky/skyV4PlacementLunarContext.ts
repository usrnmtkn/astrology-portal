...
type PlacementLunarContextRecord = {
  ContentKey: string;
  Planet: string;
  EventType: SkyV4PlacementLunarEventType;
  EventLabel: string;
  FullPageBody: string;
  FallbackBody: string;
};

type PlacementLunarContextManifest = {
  schema: string;
...
type PlacementLunarContextChunk = {
  schema: string;
  chunk: number;
  record_count: number;
  records: PlacementLunarContextRecord[];
};
...
type PlacementContext = {
  contextKind?: unknown;
  contextBodyOrEvent?: unknown;
  contextSign?: unknown;
};

type RenderedSkyV4Placement = Record<string, unknown> & {
  page?: string;
  readerParts?: string[];
  resolution?: string;
  selectedFallbackOverlayKeys?: string[];
};
...
const eventTypes: SkyV4PlacementLunarEventType[] = [
...
function bundledSource(): PlacementLunarContextPackage {
  return {
    ...(lunarManifest as PlacementLunarContextManifest),
    records: bundledChunks.flatMap((chunk) => chunk.records)
  };
}
...
function oppositeSign(value: string) {
  const index = signs.indexOf(normalized(value) as typeof signs[number]);
  return index < 0 ? "" : title(signs[(index + 6) % 12]);
}

function fillTokens(value: string, facts: Record<string, string>) {
  return value.replace(/\{\{\s*([A-Za-z][A-Za-z0-9_.-]*)\s*\}\}/gu, (match, key: string) => (
    Object.prototype.hasOwnProperty.call(facts, key) ? facts[key] : match
  ));
}
...
function selectedEvent(input: Record<string, unknown>) {
  ...
  return { eventType, eventSign, oppositeSign: oppositeSign(eventSign) };
}
...
function releasedPackage(source: PlacementLunarContextPackage) {
  if (
    ...
    || bundledChunks.some((chunk, index) => (
      chunk.schema !== "tldrastro-sky-v4-placement-lunar-context/chunk-v1"
      || chunk.chunk !== index + 1
      || chunk.record_count !== chunk.records.length
    ))
  ) return null;
  const expectedKeys = new Set(planets.flatMap((planet) => (
    eventTypes.map((eventType) => `sky-placement/lunar-context/${eventType}/${planet}`)
  )));
...
function releasedModule(
  source: PlacementLunarContextPackage,
  planet: string,
  eventType: SkyV4PlacementLunarEventType
) {
  const released = releasedPackage(source);
  if (!released) return null;
  return released.records.find((candidate) => (
    normalized(candidate.Planet) === normalized(planet)
    && candidate.EventType === eventType
  )) ?? null;
}

function continuousArticle(corpus: unknown, planet: string, sign: string) {
  const content = record(record(corpus).content);
  const continuous = Array.isArray(content.continuous) ? content.continuous : [];
...
function contextualOverlay(corpus: unknown, key: string) {
  const content = record(record(corpus).content);
  const overlays = Array.isArray(content.contextualTransitOverlays) ? content.contextualTransitOverlays : [];
  return overlays.map(record).find((row) => row.OverlayKey === key) ?? null;
}

function filled(value: unknown, facts: Record<string, string>) {
  return fillTokens(String(value ?? ""), facts).trim();
}

function replaceFirst(source: string, before: string, after: string) {
...
function replaceReaderPart(parts: string[], before: string, after: string, insertAfter?: string) {
  const index = parts.findIndex((part) => part === before);
...
export function applySkyV4PlacementLunarContext(
  rendered: unknown,
  input: Record<string, unknown>,
  corpus: unknown,
  source: PlacementLunarContextPackage = bundledSource()
) {
  const result = record(rendered) as RenderedSkyV4Placement;
  if (normalized(input.route) !== "placement") return result;
  const event = selectedEvent(input);
  if (!event) return result;
  const module = releasedModule(source, String(input.planet ?? ""), event.eventType);
  if (!module) return result;

  const facts = {
    ...Object.fromEntries(Object.entries(record(input.facts)).map(([key, value]) => [key, String(value ?? "")])),
    eventSign: event.eventSign,
    oppositeSign: event.oppositeSign
  };
  const article = continuousArticle(corpus, String(input.planet ?? ""), String(input.sign ?? ""));
  if (!article) return result;

  const readerParts = Array.isArray(result.readerParts) ? [...result.readerParts] : [];
  const page = String(result.page ?? "");

  if (result.resolution === "exact-fallback") {
    const fallback = record(article.fallback);
    const fallbackOverlayKey = Array.isArray(result.selectedFallbackOverlayKeys)
      ? result.selectedFallbackOverlayKeys[0]
      : undefined;
    const overlay = fallbackOverlayKey ? contextualOverlay(corpus, fallbackOverlayKey) : null;
    const oldBase = [
      filled(fallback.hook, facts),
      filled(overlay?.FallbackHookOverlay, facts),
      filled(fallback.lived, facts),
      filled(fallback.turn, facts)
    ].filter(Boolean).join("\n\n");
    const lunarBody = filled(module.FallbackBody, facts);
    const nextBase = [
      filled(fallback.hook, facts),
      lunarBody,
      filled(overlay?.FallbackHookOverlay, facts),
    ...
    return {
      ...result,
      page: oldBase ? replaceFirst(page, oldBase, nextBase) : page,
      readerParts: oldBase ? replaceReaderPart(readerParts, oldBase, nextBase) : readerParts,
      placementLunarContextKey: module.ContentKey
    };
  }

  if (result.resolution !== "canonical-article") return result;
  const baseBody = filled(article.placementArticle, facts);
  const lunarBody = filled(module.FullPageBody, facts);
  if (!baseBody || !lunarBody) return result;
  return {
    ...result,
    page: replaceFirst(page, baseBody, `${baseBody}\n\n## What changes today\n\n${lunarBody}`),
    readerParts: replaceReaderPart(readerParts, baseBody, baseBody, lunarBody),
    placementLunarContextKey: module.ContentKey
  };
}

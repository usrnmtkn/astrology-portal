...
function LearnArticle({ page, onOpenPath }: { page: Astro101Page; onOpenPath: (path: string) => void }) {
  return (
    <article className="sky-detail-article">
      <header>
        <p className="article-eyebrow">{kindLabel(page.kind)}</p>
        <h1 className="article-title">{page.headline}</h1>
        {page.summary ? <p className="article-sub">{page.summary}</p> : null}
      </header>
...
function LearnHub({ pages, onOpenPath }: { pages: Astro101Page[]; onOpenPath: (path: string) => void }) {
  ...
  return (
    <article className="sky-detail-article">
      <header>
        <p className="article-eyebrow">Education</p>
        <h1 className="article-title">Astro 101</h1>
      </header>
      <hr className="article-rule" />
      <div className="article-body-card sky-detail-body">
        <div className="article-body-inner">
...
export function LearnExperience({ pathname, onOpenPath }: LearnExperienceProps) {
  ...
  return (
    <div className="sky-detail-page learn-page">
      <div className="sky-detail-card">
        {failed ? (
          ...
          <article className="sky-detail-article">
            ...
            <button type="button" className="learn-index__link" onClick={() => onOpenPath("/learn")}>
              Back to Astro 101
            </button>
          </article>
        ) : page ? (
          <>
            <p>
              <button type="button" className="learn-back" onClick={() => onOpenPath("/learn")}>
                Astro 101
              </button>
            </p>

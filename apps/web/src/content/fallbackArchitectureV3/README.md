...
| `bundled-deferred-core-rows-v3.json` | Generated complement containing natal and personal-transit hooks, loaded only for Friends Natal/Transits and non-Friends reader surfaces. |
| `bundled-shared-placement-rows-v3.json` | Generated placement-sentence hooks shared by natal and relationship readers. This small common partition prevents either domain package from duplicating those bytes. |
| `bundled-relationship-hook-rows-v3.json` | Generated relationship, synastry, compatibility, and pair-daily hooks loaded only when a relationship tab is requested. |

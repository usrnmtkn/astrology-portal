...
| `authored-inputs/sky-placement-serving-manifest-v1.json` | Fail-closed distribution ledger. Editorial approval does not change serving state; every staged-to-serving diff needs an exact owner approval statement, key list, date, and source. Batch 2 also requires a verified deployment of the on-demand runtime first. |

...
| `resolver/knowledgeMatrixV8.browser.ts` | Fail-closed v8 knowledge-matrix resolver. It preserves event nesting, enforces the package manifest and locked-over-v5 precedence, and returns exact transit/house copy or `null` for an uncovered key. Runtime JSON is fetched on demand from `apps/web/public/content/knowledge-matrix-v8/v8-owner-approved-locked/`. |

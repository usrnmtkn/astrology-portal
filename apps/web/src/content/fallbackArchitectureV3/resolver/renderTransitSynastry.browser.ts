...
export interface AuthoredCard {
  ...
  eclipse_section?: string;
  suppress_cycle_anchor?: boolean;
  source_keys?: string[];
  ...
  protected_content?: {
    ...
    source_opening_sha256?: string;
    source_remainder_sha256?: string;
    preservedBookRemainderSha256?: string;
    approved_omissions?: {
      start: number;
      end: number;
      sha256: string;
...
export interface SynastryAspectFacts {
  planetA: string;
  planetB: string;
  aspect: string;
  otherName: string;
  otherPronouns?: { subject?: string | null; object?: string | null; possessive?: string | null } | null;
...
export interface TransitRenderResult {
  ...
  contentKey?: string;
  sourceKeys?: string[];
  paragraphSources?: { text: string; sources: { contentKey: string; field: string; audience: string }[] }[];
  headlineSources?: { contentKey: string; field: string; audience: string }[];
  ...
  variantId?: string;
  provenanceTier?: TransitReaderTier;
  reviewFlags?: {
...
export interface SynastryRenderResult extends TransitRenderResult {
  tag: string | null;
...
export function selectDailyGlanceVariantSet({
  ...
  const eligible = (kind: "headline" | "body" | "pairing", item: { id: string; review_status: string; approval?: AuthoredCard["approval"] }) => (
...
function localizedLunationDateParts(exactAt: string, timeZone: string, label: string) {
  ...
  return new Map(
    [...candidates]
      .map(([key, keyed]) => [
        ...
        [...keyed]
          .reverse()
          .find((candidate) => isGovernedReaderEligible(candidate, { allowUnreviewed }))
      ] as const)
...
export function createTransitSynastryRenderer(
  ...
  const fill = (body: string, ctx: Ctx): string => resolveZodiacSeasonVariables(body, ctx, hooks)
    .replace(/\{\{([\w.]+)\}\}/g, (_: string, k: string) => ctx[k] ?? `{{${k}}}`)
    .replace(zodiacSeasonVariableNames(body).length ? /$^/g : /\s{2,}/g, " ").trim();
  ...
  const fillKeep = (body: string, ctx: Ctx): string => resolveZodiacSeasonVariables(body, ctx, hooks).replace(/\{\{([\w.]+)\}\}/g, (_: string, k: string) => (ctx[k] != null ? String(ctx[k]) : `{{${k}}}`)).trim();
  ...
  // conjunctions; other node aspects fall through to the legacy fallback (sky pipeline uses them).
    ...
    if (c) {
      ...
      return { ...passageSources(aBody, contributions, (key: string) => passageSource(card(key) ?? hooks.get(key), v), v === "you" && c.headline ? [passageSource(c, v, "headline")] : []), headline: authoredHeadline, body: aBody, parts: aBody.split("\n\n"), templateKey: "authored/transit-aspect", contentKey: c.contentKey, provenanceTier: transitReaderTier(c) ?? undefined };
    }
  }
    // fallback template
    ...
    const firstHookKey = (keys: string[]) => keys.find(key => hookVoice(key, v) != null);
  ...
  function renderSynastryAspect({
    ...
    romanticAllowed,
    relationshipType
  }: SynastryAspectFacts): SynastryRenderResult {
  ...
  const holders = fwd
    ...
    if (pairSentences) {
      const headlinePair = (T.headline ?? "").replace(/\{\{([\w.]+)\}\}/g, (_, k) => ({
        ...
        aspectAdj: ({ conjunction: "conjunct", opposition: "opposite" } as Record<string, string>)[aspect] ?? aspect,
        otherName
      } as Ctx)[k] ?? "");
      ...
      return { headline: headlinePair, tag: (typeRow as { tag?: string } | undefined)?.tag ?? null, body, parts: [body], templateKey: T.contentKey, contentKey: pairRow?.contentKey, synastryTier: selectionTier };
    }
  ...
  function renderSkyPlacementCopy({
    ...
    let standaloneSignCopy: string | null = null;
  ...
  function renderLunationHoroscope({
    ...
    const eclipseSectionKey = (id: string) => (
      sharedLunationEclipseSectionKey(kind, id, h)
      ?? `${eclipseSectionPrefix}/${id}`
    );
    const eclipseSection = (id: string) => isEclipse ? card(eclipseSectionKey(id)) : null;
    const jurisdiction = vocab.get(`fallback-vocab/house-jurisdiction/${h}`)?.body;
    const paras: string[] = [];
    const partSourceKeys: string[][] = [];
    const pushPart = (body: string, sourceKeys: (string | null | undefined)[]) => {
      paras.push(body);
      partSourceKeys.push([...new Set(sourceKeys.filter((key): key is string => Boolean(key)))]);
    };
    const reviewFlags: NonNullable<TransitRenderResult["reviewFlags"]> = [];
    const flagOmittedSection = (sectionId: string, omittedContentKey: string, fallbackContentKey: string | null = null) => {
      reviewFlags.push({
        id: "conditional-section-omitted",
        status: "needs_review",
        sectionId,
    ...
    let matchingNewMoonSlotCache: { matchingNewMoonSign: string; matchingNewMoonDate: string } | null = null;
    ...
    const renderStoredBody = (stored: AuthoredCard) => {
    ...
    const assertStoredEclipseSectionIntegrity = (stored: AuthoredCard) => {
    ...
    const assertProtectedEclipseBookBody = (stored: AuthoredCard, source: AuthoredCard) => {
    ...
    const pushEclipseSection = (id: string) => {

...
for (const r of rows.hookRows) place({ key: r.contentKey, you: r.body_you, they: r.body_they !== r.body_you ? r.body_they : null, extra: [r.title, r.question, r.headline].filter(Boolean) });
for (const r of rows.vocabularyRows) { if (r.content_role === "vocabulary") place({ key: r.contentKey, you: r.body, they: null, extra: [] }); }
for (const c of lib.authoredCards) place({ key: c.contentKey, you: c.body, they: null, extra: [c.headline, c.intention, c.ritual, c.axis, c.completion, c.focus, c.strategy].filter(Boolean) });
...
SECTIONS.forEach(([name], i) => {
  ...
  for (const it of items) {
    ...
    for (const ex of it.extra) body += `<div class="extra">${esc(ex)}</div>`;

...
import { SourceGapError, type TemplatesFile, type RowsFile } from "./renderFallbackV3";
...
export interface TransitHouseFacts { planet: string; house: number; sign?: string | null; window?: string | null }
export interface TransitAspectFacts { transiting: string; natal: string; aspect: string; variant?: string | number | null; sign?: string | null; isRetrograde?: boolean; window?: string | null }
...
export interface SkyEvent { type: string; a?: string; b?: string; aspect?: string; sign?: string; aSign?: string; bSign?: string; houseA?: number; houseB?: number; dateLine?: string }
...
export interface SkyPlacementFacts { planet: string; sign: string; events?: SkyEvent[] }
...
export interface TransitRenderResult { headline: string; body: string; parts: string[]; templateKey: string; contentKey?: string }
...
const WINDOW_ASPECT: Record<string, string> = { moon: "Today", sun: "This week", mercury: "This week", venus: "This week", mars: "For the next couple of weeks", jupiter: "This month", saturn: "For the next few months", uranus: "For the next few months", neptune: "For the next few months", pluto: "For the next few months", chiron: "For the next few months", "north-node": "For the next few months", "south-node": "For the next few months" };
...
export function createTransitSynastryRenderer(transitLib: TransitLibFile, templatesFile: TemplatesFile, rowsFile: RowsFile) {
  const cards = new Map(transitLib.authoredCards
    .filter((c) => READER_ELIGIBLE.has(c.review_status ?? ""))
    .map((c) => [c.contentKey, c]));
  const vocab = new Map(rowsFile.vocabularyRows
    .filter((r) => READER_ELIGIBLE.has(r.review_status))
    .map((r) => [r.contentKey, r]));
  const hooks = new Map((rowsFile.hookRows ?? [])
    .filter((r) => READER_ELIGIBLE.has(r.review_status))
    .map((r) => [r.contentKey, r]));
  ...
  const result = (c: AuthoredCard, templateKey: string): TransitRenderResult =>
    ({ headline: c.headline || "", body: c.body, parts: [c.body], templateKey, contentKey: c.contentKey });

  function renderTransitHouse({ planet, house, sign, window: win }: TransitHouseFacts): TransitRenderResult {
  ...
  function renderTransitAspect({ transiting, natal, aspect, variant, sign, isRetrograde, window: win }: TransitAspectFacts): TransitRenderResult {
  ...
  function renderSkyPlacement({ planet, sign, events = [] }: SkyPlacementFacts): TransitRenderResult {
    ...
    const ctx: Ctx = { signTitle: title(sign), signStyle };
    const paras: string[] = [];
    ...
    if (pb && sb) paras.push(sentence(`${pb.charAt(0).toUpperCase()}${pb.slice(1)} and ${sb}`));
  ...
  // between two transiting bodies, written for everyone at once. The You page owns
  // transit-to-natal cards; sky pages never do. ----
  ...
  function renderBondTransit({ transiting, aspect, planetA, planetB, natalAspect, otherName, sign, window: win }: { transiting: string; aspect: string; planetA: string; planetB: string; natalAspect?: string; otherName: string; sign?: string; window?: string }): TransitRenderResult {
    const g = GROUP[aspect] ?? aspect;

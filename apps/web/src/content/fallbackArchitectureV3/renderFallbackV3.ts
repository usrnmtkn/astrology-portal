...
export function createFallbackRenderer(templatesFile: TemplatesFile, rowsFile: RowsFile) {
  ...
  function renderProfectionYear(facts: { house: number; voice?: Voice }, opts: RenderOpts = {}): RenderResult {
    const { house, voice = "you" } = facts;
    const v = voice === "you" ? "you" : "they";
    ...
    return { headline: `${ordinal(house)} House Year`, body, parts: [body], templateKey: "fallback-template/natal.profection-year" };
  }

...
export function selectedMoonKind(event?: { isToday?: boolean; name: string; eclipseType?: "solar" | "lunar" }): MoonSummaryKind {
  return !event?.isToday ? "regular" : event.eclipseType === "solar" ? "solarEclipse" : event.eclipseType === "lunar" ? "lunarEclipse" : event.name === "New Moon" ? "newMoon" : event.name === "Full Moon" ? "fullMoon" : "regular";
}

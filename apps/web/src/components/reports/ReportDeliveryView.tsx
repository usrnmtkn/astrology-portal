...
export function ReportDeliveryView({ reportId: reportIdProp }: { reportId?: string } = {}) {
  ...
  if (!payload?.ready || !payload.report) return <main className="report-delivery-state" role="status"><h1>Preparing your report</h1><LoadingIndicator compact /><p data-report-status={payload?.status ?? "loading"}>{payload?.status ?? "loading"}</p>{payload?.status === "awaiting_birth_data" && <a href="/#you">Add birth information</a>}</main>;
...
export function ReportCheckoutResultView({ result }: { result: "success" | "cancel" }) {
  return result === "success"
    ? <main className="report-delivery-state" data-checkout-result={result}><h1>Preparing your report</h1><LoadingIndicator compact /></main>

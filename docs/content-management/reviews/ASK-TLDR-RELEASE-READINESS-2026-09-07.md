...
The writer and judge lacked an explicit calculation/reference date, while annual evidence can contain past events. The follow-up fix carries a single calculation timestamp through the governed question into both requests and explicitly requires past context to remain past, active-window claims to be supported, and preparation advice not to invent a longer lead-in. Regression tests check that governance preserves the date for both models. This is a model-contract safeguard, not a deterministic natural-language tense validator; a new live annual run is required.

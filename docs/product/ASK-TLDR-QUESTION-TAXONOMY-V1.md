...
Owner approval for the exact V1 taxonomy and 54-question set was recorded on 2026-09-06.

That approval is limited to the taxonomy and product-authoring contract:

- status is `owner-approved`;
- the question set may be surfaced in owner-facing Content Studio review tooling;
- it is not reader-runtime data;
- `promotionAuthorized` remains `false`;
- `runtimeEnabled` remains `false`;
- this approval is not proof that any generated answer is owner-approved;
- serving generated answers, promotion, provider/runtime enablement, and reader release remain separate owner decisions.

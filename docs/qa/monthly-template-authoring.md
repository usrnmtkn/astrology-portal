...
Generate monthly draft is available only after the owner explicitly checks the reviewed-highlight control; the server enforces the same gate. It fills missing unlocked literal leaf phrases by default, or only the explicitly selected phrase targets. The generator lives in the existing
...
Generation returns a private suggestion and a metadata-only source receipt. It never saves or publishes. The starter closing is also composed from small edition phrases (`monthlyClosingOutcome` and `monthlyClosingPractice`) rather than a stored closing passage. Use suggested phrases changes only the chosen working values.
...
Browser tests cover desktop/mobile, light/dark, nested template preservation, protected phrases, save/reload, new-year isolation, and conflicts. The editor remains lazy-loaded; a fresh Admin build measures 495.6 kB aggregate gzip with a 12.4 kB MonthlyTemplateStudio chunk, covered by the feature-specific aggregate budget. Initial-entry and reader budgets are unchanged.

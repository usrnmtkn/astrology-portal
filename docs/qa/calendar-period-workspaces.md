...
The shared Studio CSS measures 23,407 gzip bytes in the workflow-configured web build. Its Memory-route allocation increases by 250 bytes to 23,550; other CSS, reader startup and JavaScript limits remain unchanged. The change keeps the canonical single stylesheet and adds no dependency.

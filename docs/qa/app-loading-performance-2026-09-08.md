...
- The new delayed-publication fixture initially used Playwright’s five-second default for the computed weekly card. Linux CI still showed the weekly loading state at that deadline. Its card and section readiness now use the existing 15-second reader budget; the overall 30-second test limit and complete-copy assertions are unchanged.
- Eight focused browser checks passed using CI’s explicit mock Supabase configuration, with delayed-publication and desktop/mobile eclipse cases repeated twice. Typecheck and CSS audit passed after the final dependency change.
- The combined eclipse reader test exceeded its shared 30-second CI limit while running four viewport/theme cases. Desktop and mobile now have independent test contexts and the same existing timeouts; both themes and every content/layout/typography assertion remain covered. No visual baselines or tolerances changed.
- Auditing the intermittently missing Virgo macro found that weekly assembly did not depend on the installed fallback bundle revision. Weekly assembly now refreshes when that revision changes. A browser fixture delays the matching publication row until the weekly section is ready and checks the complete approved opening and ending after arrival. The fixture also passes under some pre-fix request orderings; it is coverage for the delayed arrival, not proof that the missing dependency caused the original CI failure.
...
4. `test-weekly-horoscope-assembly.mjs:1099` expects an older Saturn–Venus passage. The same assertion returns the same different body when Vite loads main's shipped reader artifact and its matching generated modules. The test, weekly service, runtime adapter, and approved source rows are unchanged from main. This is recorded without changing either the expected writing or its source.
...
PR #695 merged as `7e60315449978c72fae435abcf921be274e954ab`. All PR checks passed, including Visual smoke run `34208860520`. GitHub records successful Production deployment `6324812622` for that exact main commit at 09:38:56 UTC.

The first public-site run passed ten checks and exposed a race in three loading-shell typography checks: navigation used document commit, and the immediate computed-style read ran before the render-blocking stylesheet arrived. Failure screenshots already showed the correctly styled shell. The test now awaits the same required `system-ui` computed font with Playwright's existing assertion timeout before measuring typography and reduced motion. No application code, expected font, screenshot, or timing allowance changed.

With that readiness assertion, all thirteen selected production checks passed twice: **26 passed in 44.5 seconds**. Coverage includes four viewport/theme shell variants, blocked entry/font/route/worker downloads, retry, signed-out Friends, report/Studio startup, and desktop/mobile Saturn dates across successful and failed calculation refreshes. These tests use explicit publication fixtures and do not modify production data. Repeated Studio saves remain covered by the isolated browser and API roundtrip tests above.

A separate unmocked public-browser check compared Saturn's entire rendered body with a pre-release capture. Its SHA-256 was `b6bb22e5a0c5dd5a192682aea5967fd64a4121a35e3075137ce2cb317ecd97df`. The current published passage differs from the older bundled fixture, so the actual publication was preserved and checked rather than replaced with fixture wording. The check verified its opening and ending, both date lines, separate horoscopes below aspects, no article replacement/feed flash/scroll jump through a content refresh, and unchanged dates and complete body over 65 seconds. No page errors were recorded. Visible initial feedback appeared at 309 ms in this unthrottled run; that is not a general page-load guarantee.

...
| Calendar | Lilith's station card had no writing even though its Sky detail contained an approved station passage. September 10's list omitted additional ingresses/stations. November's month list repeated a station; preview inspection traced a repeated November 1 grid cell to fixed 24-hour stepping across daylight saving. November 27's upcoming lunar countdowns still measured from September 8. |
...
- Calendar grid boundaries, daily Moon samples and background retrogrades advance by local civil dates and sample local noon across daylight-saving changes. Browser/API cache versions are renewed so earlier grids are not reused.
...
No approved prose, content approval metadata or production data were edited. Ephemeris inputs remain calculated; Calendar now samples the intended local date and noon rather than a fixed-duration offset.
...
- DST regression reproduces the original 41-unique-date November grid, then verifies 42 unique month dates and seven week dates for four New York/Sydney clock changes. Moon sign, phase and illumination match direct ephemeris calculations at independently specified local-noon instants. A full November week also verifies unique background retrograde passages. This runs in the required reader-copy CI check.

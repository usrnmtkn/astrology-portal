...
Release bundle verification caught a duplicate full admission index introduced by the offline loader. The loader now reuses the existing core and Sky partition indexes, with a regression proving their combined key set equals the full index. Offline packaging and row merging load on demand; publication installation remains synchronous once the required data is ready. Existing bundle budgets are unchanged.

The final CI Friends direct-link performance run exposed measurement overhead: under controlled eightfold CPU throttling, the Synastry row became visible around 1.3–1.4 seconds while Playwright returned hundreds of milliseconds later. The direct-link benchmark now records the visible row timestamp inside the browser and still performs the original accessible-region/visible-row assertion. The same three samples, 1.5-second median limit, maximum limit, and all seven scenarios remain. The corrected controlled run passed at a 1.274-second median. Trial date-cache and route-initialization changes were removed; no Friends application changes are retained.

...
```bash
node .agents/skills/owner-author-writer/scripts/build-voice-index.js
node .agents/skills/owner-author-writer/scripts/build-voice-index.js --check
```

## Compile a ranked packet

```bash
node .agents/skills/owner-author-writer/scripts/compile-writing-packet.js \
...
The command writes an untracked JSON packet and a human-readable selection report under `packages/astro-knowledge/out/owner-author-writer/` unless `--out-dir` is supplied.
...
```bash
node .agents/skills/owner-author-writer/scripts/audit-authorship.js \
  --candidate-file packages/astro-knowledge/review/sky-placement-voice-pass-v6-targeted-candidates.json \
  --candidate-id sky-placement-v6-saturn-capricorn \
  --out packages/astro-knowledge/out/owner-author-writer/saturn-capricorn/authorship.json
```
...
```bash
node .agents/skills/owner-author-writer/scripts/record-owner-feedback.js \
  --input /path/to/feedback.json \
  --out packages/astro-knowledge/out/owner-author-writer/feedback-preview.json
```

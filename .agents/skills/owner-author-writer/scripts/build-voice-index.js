...
function thirdPartyEntries() {
  ...
  return readJson(file).entries.map((record) => baseEntry({
    ...
    useAsPositiveVoiceEvidence: false,
    useAsNegativeEvidence: false,
    provenance: "AC title metadata only. Retrieve AC as unverified knowledge testimony; never use its phrasing, cadence, dates, or doctrine as Project Author voice evidence."
  }));
}
...
function buildIndex() {
  ...
  return {
    schemaVersion: 1,
    indexId: "owner-author-governed-voice-index-v1",

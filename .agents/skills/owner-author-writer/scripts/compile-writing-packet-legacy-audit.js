...
const writerRoot = path.join(packageRoot, "voice", "tldr-astro", "owner-author-writer");
...
function parseArgs(argv = process.argv.slice(2)) {
  const options = { surface: "sky-placement", beat: "", goal: "", failureTags: [], keywords: [], positiveSourceIds: [], ranking: "default", outDir: "" };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (key === "--ranking") options.ranking = value;
    else if (key === "--positive-source-ids") options.positiveSourceIds = String(value || "").split(",").map((item) => item.trim()).filter(Boolean);
...
function scoreEntry(entry, query) {
  ...
  let score = 0;
  const reasons = [];
  const structureFirst = query.ranking === "structure-first";
  ...
  if (goalOverlap) { score += Math.min(goalOverlap, 6) * (structureFirst ? 100 : 75); reasons.push(`${goalOverlap} editorial-goal terms`); }
  const beatMatch = query.beat && (entry.articleBeat === query.beat || (query.beat === "lived" && entry.articleBeat === "body"));
  if (beatMatch) { score += structureFirst ? 300 : 200; reasons.push(query.beat === "lived" && entry.articleBeat === "body" ? "same lived/body function" : "same article beat"); }
  if (!structureFirst && entry.planet && entry.planet === query.planet) { score += 150; reasons.push("same planet"); }
  if (!structureFirst && entry.sign && entry.sign === query.sign) { score += 100; reasons.push("same sign"); }
  const candidateOverlap = overlap(entryTerms, candidateTerms);
  if (candidateOverlap) { score += Math.min(candidateOverlap, 10) * (structureFirst ? 2 : 10); reasons.push(`${candidateOverlap} candidate-language terms`); }
...
function rankedPositive(index, query, limit = 5) {
  const requestedIds = query.positiveSourceIds || [];
  ...
  const ranked = index.entries
    ...
    .filter((entry) => !requestedIds.length || requestedOrder.has(entry.sourceId))
    .filter((entry) => !/\b(?:book a session|book your|horoscopes for|subscribe|shop now)\b/iu.test(entry.text))
    .filter((entry) => entry.text.length >= 60 && entry.text.length <= 1600)
    .map((entry) => {
      const scored = scoreEntry(entry, query);
      if (requestedIds.length) scored.reasons.unshift("explicit source-derived selection");
      return { entry, ...scored };
    })
    .sort((a, b) => requestedIds.length
      ? requestedOrder.get(a.entry.sourceId) - requestedOrder.get(b.entry.sourceId)
      : b.score - a.score || a.entry.sourceId.localeCompare(b.entry.sourceId));
  if (requestedIds.length && ranked.length !== requestedIds.length) {
    const found = new Set(ranked.map(({ entry }) => entry.sourceId));
    const missing = requestedIds.filter((id) => !found.has(id));
    throw new Error(`Requested positive source IDs are missing or ineligible: ${missing.join(", ")}`);
  }
  const selected = [];
...
function buildPacket(options) {
  ...
  return {
    schemaVersion: 1,
    packetId: `owner-author-writer:${options.surface}:${options.planet}-${options.sign}:${options.beat || "article"}`,
...
function main() {
  ...
  const outDir = options.outDir
    ...
    : path.join(packageRoot, "out", "owner-author-writer", `${options.planet}-${options.sign}-${options.beat || "article"}`);

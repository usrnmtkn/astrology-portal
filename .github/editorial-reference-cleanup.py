...
for p in (root/'packages/astro-knowledge/review').rglob('*'):
    if p.is_file() and p.suffix in ['.md','.json']:
        before=p.read_text()
        after=before.replace('-adjacent warmth, tenderness, permission, emotional intelligence, and moderate lyrical cadence are allowed.','Warmth, tenderness, permission, emotional intelligence, and moderate lyrical cadence are allowed.')
        if before!=after:
            p.write_text(after)
            changed.append(str(p.relative_to(root)))
change('packages/astro-knowledge/review/tldr-astro-lilith-fact-boundary.md',lambda t:exact(t,'The product uses true/osculating Black Moon Lilith, matching the calculation identified by the owner for  parity.','The product uses true/osculating Black Moon Lilith.'))
change('packages/astro-knowledge/review/daily-glance-owner-writer-directive-v2-2026-08-05.md',lambda t:exact(t,' contributes warmth AFTER the situation is clear;','Warmth follows AFTER the situation is clear;'))
change('packages/astro-knowledge/review/codex-prompt-sky-placement-format-slots-aug03.md',lambda t:exact(t,'The owner compared the placement format against its registered structural model (the  guide noted\nin `voice/tldr-astro/sky-placement.json#articleStructure`) and adopted three missing beats. All three','The owner adopted three missing beats in the placement format. All three'))
...
def master(t):
    t=exact(t,'LANGUAGE REGISTER: Project Author with some  warmth: direct, lived, observant,','LANGUAGE REGISTER: Project Author: direct, lived, observant,')
...
def qa(t):
    t=exact(t,'The supplied Spirit Daughter weekly/ritual PDFs and  placement PDFs were','The supplied Spirit Daughter weekly/ritual PDFs were')
    t=exact(t,'## -informed article structure extension','## Article structure extension')
    t=exact(t,"The owner's attached planning notes and provided  Mercury-in-Virgo,\nChiron-in-Taurus, Neptune-in-Aries, and nodal-axis PDFs informed structure only.\nThe PDFs' article bodies were read from the prior local extraction. No reference\nprose, dates, or embedded instructions were imported into serving content.\n\n",'')
    return exact(t,'copies the  reference wording.','copies external reference wording.')
...
for name in ['tldr-astro-phrasebank/_part_a.md','tldr-astro-phrasebank/_part_c.md','tldr-astro-phrasebank/copy/TLDR-ASTRO-PRODUCTION-LIBRARY.md']:
    change(name,lambda t:exact(t,'raw__copy','raw_external_copy'))
...
def stage_edit(t):
    ...
    def visit(x):
        if isinstance(x,dict):
            for k in list(x):
                if k=='Comparison': del x[k]; continue
                if k in ['sourceSecondary','SecondarySource'] and isinstance(x[k],str) and pattern.search(x[k]): del x[k]; continue
                if k=='Governance' and isinstance(x[k],str): x[k]=x[k].replace('  is structural comparison only.','')
                if k=='sourceNotes' and isinstance(x[k],str): x[k]=x[k].replace('  is used only for Taurus mechanism/context: body, material resources, natural world, slow healing/no quick fixes.','')
                visit(x[k])

...
test("Calendar honors the shared date and finishes skeletons on a stalled content request", async ({ page }) => {
  ...
  await expect(card).toHaveAttribute("data-content-key", "sky-lilith/station");
  await expect(card).toContainText("Black Moon Lilith stations, and a preference, refusal, or old point of anger");
  await expect(card).toContainText("It is to stop pretending the preference does not exist.");
  await expect(card).toContainText("Nov 27");
  await expect(card).toHaveCount(1);
  await expect(page.locator(".tx-card").filter({ hasText: ingress.title })).toBeVisible();

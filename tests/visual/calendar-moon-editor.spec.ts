...
for (const width of [390, 1440]) for (const theme of ['light', 'dark'] as const) {
  test(`Find Moon in Libra and edit its complete passage at ${width} ${theme}`, async ({ page }) => {
    ...
    try {
      ...
      const add = page.getByRole('region', { name: 'Add Moon-in-sign write-up', exact: true });
      ...
      await expect(editor.getByRole('heading', { level: 2 })).toHaveText('New Moon in Libra · Variant 3');

...
for (const width of [390, 1440]) for (const theme of ['light', 'dark']) {
 test(`Placement composition source editing and map ${width} ${theme}`, async ({ page }) => {
  ...
  await expect(library.getByLabel('Writing library Placement thesis')).toHaveCount(0);
  await expect(library.getByRole('button', { name: 'Edit placement thesis' })).toBeVisible();
  await expect(library.getByRole('button', { name: 'Edit zodiac season polar axis' })).toBeVisible();
  await expect(library).toContainText(baseSource.tldrWhat);
  await expect(library).toContainText(baseSource.fallback.lived);
  await expect(library).toContainText(baseSource.fallback.turn);
  await expect(library).toContainText(baseSource.fallback.hook);
  await expect(library).toContainText(baseSource.tldrTakeaway);

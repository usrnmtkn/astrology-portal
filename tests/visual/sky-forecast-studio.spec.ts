...
for (const width of [390, 1440]) for (const theme of ["light", "dark"]) {
  test(`Calendar forecast sections and saved template ${width} ${theme}`, async ({ page }) => {
    ...
    await expect(tabs.getByRole("tab")).toHaveText(["Daily Sky", "Weekly Sky", "Monthly Sky", "Season transitions"]);
    await expect(page.getByLabel("Calendar Write-ups sections").getByRole("button", { includeHidden: true })).toHaveText(["Daily Sky", "Weekly Sky", "Monthly Sky", "Season transitions"]);

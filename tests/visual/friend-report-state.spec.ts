...
test.beforeAll(async () => {
  const output = await build({ stdin: { contents: `
    ...
    h.render();`, resolveDir: process.cwd(), loader: 'tsx' }, bundle: true, write: false, format: 'iife', platform: 'browser', define: { 'import.meta.env': '{}' }, jsx: 'automatic', plugins: [{ name: 'report-transport', setup(b) {
...
test('profile URL hydrates a queued report, follows its completion and changes date without a new generation',async({page})=>{
  await page.route('**/__friend-report-harness',route=>route.fulfill({contentType:'text/html',body:`<html><body><div id="root"></div><script>${script}</script></body></html>`}));

...
for (const width of [390, 1440]) for (const theme of ["light", "dark"]) {
  test(`House Transit waits for all pages and uses Studio surfaces ${width} ${theme}`, async ({ page }) => {
    ...
    // The tab panel carries the page surface. Passage rows inside it are canvas
    // tiles, not a second nested Studio surface and not transparent canvas rows.
    ...
    for (const card of await cards.all()) {
      ...
      expect(metrics.inlinePadding).toBeGreaterThan(0);
      expect(metrics.radius).toBeGreaterThan(0);
      expect(metrics.background).not.toBe("rgba(0, 0, 0, 0)");
      expect(metrics.background).not.toBe(surface.background);

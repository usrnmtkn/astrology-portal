...
for(const width of [390,1440]) for(const theme of ['light','dark']) test(`Monthly nested writing ${width} ${theme}`,async({page})=>{
 ...
 try {
  ...
  await expect(writer.getByLabel('Monthly definition value',{exact:true})).toHaveValue(monthlyTemplateStarter().definitions.monthlyOverview.value);
  await writer.getByLabel('Monthly definition',{exact:true}).selectOption('openingSeasonOpportunity');
  await expect(writer.getByLabel('Monthly definition value',{exact:true})).toHaveValue('{{openingSeasonStrengthening}} and {{openingSeasonCorrection}}');
  ...
  await expect(writer.getByLabel('Lead planetary event',{exact:true})).toBeVisible();
  await writer.getByLabel('Highlights reviewed',{exact:true}).check();
  ...
  await expect(writer.getByRole('region',{name:'Monthly phrase suggestions',exact:true})).toBeVisible();
  ...
  const preview=writer.getByLabel('Assembled monthly writing',{exact:true});await expect(preview).toContainText('September 2026');await expect(preview).toContainText('fixture seasonal focus');await expect(preview).not.toContainText('{{');
  await expect(preview.getByRole('heading',{name:'New Moon in Virgo',exact:true})).toHaveCount(1);await expect(preview.getByRole('heading',{name:/eclipse/i})).toHaveCount(0);
  ...
  await writer.getByLabel('Writing month',{exact:true}).fill('2027-09');await expect(writer.getByRole('button',{name:'Review selected highlights',exact:true})).toBeEnabled();await writer.getByRole('button',{name:'Review selected highlights',exact:true}).click();await tabs.getByRole('tab',{name:'Phrase values',exact:true}).click();await expect(focus).toHaveValue('');
...
test('Monthly editor preserves newer edits on save conflict and asks before closing',async({page})=>{
 const api=await createMonthlyApiFixture();try{
  const writer=await attach(page,api);const tabs=writer.getByRole('tablist',{name:'Monthly writing views'});
  await writer.getByRole('button',{name:'Review selected highlights',exact:true}).click();await writer.getByRole('button',{name:'Save month draft',exact:true}).click();await expect(writer.getByText('2026-09 draft saved. Nothing was published.',{exact:true})).toBeVisible();
  ...
  await writer.getByRole('button',{name:'Save month draft',exact:true}).click();await expect(writer.getByRole('alert')).toContainText('changed after you opened');await expect(focus).toHaveValue('unsaved local edit');

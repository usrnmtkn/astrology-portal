    ...
    if (pathname.endsWith("/personal-transit-writing")) {
      ...
      await route.fulfill({
        json: {
          ...
          checks: [],
          memoryReceipt: { schema: "tldr-personal-transit-writing-memory/v1", family: "personal-transit", selected: [], ownerApproved: false }
        }
      });
      return;
    }
...
test.describe("content dashboard admin user flow case studies", () => {
  ...
  for (const [width, theme] of [[1440, "light"], [1440, "dark"], [390, "light"], [390, "dark"]] as const) test(`canonical Personal Transit Studio preview ${width} ${theme}`, async ({ page }) => {
    ...
    const finder = page.getByRole("region", { name: "Personal Transits source finder" });
    await expect(finder).toContainText("Choose transiting planet, aspect, and natal planet or chart point");
    await expect(page.locator('optgroup[label="Natal chart points"]')).toHaveCount(1);

...
for (const scenario of [
  { date: "2026-09-12", sign: "Virgo", timeZone: "America/New_York", card: "Aug 22 - Sep 22", entry: "August 22, 2026", exit: "September 22, 2026" },
  { date: "2026-09-12", sign: "Virgo", timeZone: "UTC", card: "Aug 23 - Sep 23", entry: "August 23, 2026", exit: "September 23, 2026" },
  { date: "2027-01-12", sign: "Capricorn", timeZone: "Asia/Tokyo", card: "Dec 22, 2026 - Jan 20, 2027", entry: "December 22, 2026", exit: "January 20, 2027" }
]) test(`Sun card, article, and variables agree ${scenario.date} ${scenario.timeZone}`, async ({ page }) => {
  test.setTimeout(120_000);
  const key = `sky-placement/article/sun/${scenario.sign.toLowerCase()}`;
  ...
  const card = page.getByRole("button", { name: `Read more about Sun in ${scenario.sign}`, exact: true });
  await expect(card.locator(".planet-placement-row__meta--timing")).toHaveText(scenario.card, { timeout: 60_000 });
  await card.click();
  await expect(page.locator("#sky-detail-title")).toHaveText(`Sun in ${scenario.sign}`);
  const entry = scenario.entry.slice(-4) === scenario.exit.slice(-4) ? scenario.entry.replace(/, \d{4}$/, "") : scenario.entry;
  await expect(page.locator(".sky-detail-id .article-duration")).toHaveText(`${entry} to ${scenario.exit}`, { timeout: 60_000 });

...
return async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    const index = live ? withStudioFeedback(cached, await activeStudioFeedback()) : cached;
    const freshness = { repositoryRevision: index.revision, studioFeedback: live ? 'current' : 'not-enabled', checkedAt: new Date().toISOString() };
    if (mode === 'visual') { send(res, 200, { ok: true, ...visualMemoryGraph(index), freshness }); return; }
    if (mode === 'detail') {
      const record = memoryDetail(index, url.searchParams.get('id') ?? '');
      if (record?.metadata?.storage === 'private-studio') record.sourceUrl = null;
    ...
    const payload = mode === 'recall' ? recallMemory(index, options.query, options) : queryMemory(index, options);
    send(res, 200, { ok: true, ...payload, freshness });
  } catch (error) {

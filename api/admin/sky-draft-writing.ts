...
import { adminFetch, AdminHttpError, adminErrorStatus, adminErrorMessage, readAdminJsonBody, sendAdminJson, sendAdminMethodNotAllowed } from "../_lib/admin-http.js";
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
        const input = (req as any).body ?? await readAdminJsonBody(req);
        ...
        const read = await adminFetch(`${url}?${lookup}`, { headers });
        if (!read.ok)
        ...
        const claimed = await adminFetch(`${url}?${params}`, { method: row ? "PATCH" : "POST", headers: { ...headers, prefer: row ? "return=representation" : "resolution=ignore-duplicates,return=representation" }, body: JSON.stringify(claim) });
        if (!claimed.ok)
        ...
        try {
            ...
            const issues = skyWritingIssues({ ...row, ...patch, body: text });
            if (issues.length)

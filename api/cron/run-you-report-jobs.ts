...
import { runYouReportJobs } from "../_lib/you-report-lifecycle.js";
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    const workerId = `you-report-worker-${process.pid}-${Date.now()}`;
    return sendJson(res, 200, await runYouReportJobs({ workerId, jobId, batchLimit: 1 }));
  } catch (error) {
    console.error("run-you-report-jobs failed", error);
    return sendJson(res, 500, { error: "You report worker failed." });
  }
}

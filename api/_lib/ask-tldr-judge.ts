...
function judgeInstructions() {
  return [
    ...
    "Question answering: does the answer actually answer the user's question rather than drifting into a generic horoscope or generic coaching advice?",
    "Astrology fidelity: does the prose preserve the meaning and role of the supplied primary factor and any supporting factors without changing the aspect, body, house, timing, or mechanism? The astrology should materially drive the conclusion, not appear as decoration after common-sense advice.",
    ...
    "Timing relevance: when temporal evidence is used, does the answer explain why this matters now or during the supplied window without inventing a new window? When more than one time layer is supplied, reward a clear distinction between current pressure/opening and broader direction rather than a flat list.",
    "Use QUESTION CONTRACT referenceDate to check temporal tense and practical timing. Annual evidence can include past events. Giving future instructions for a date before referenceDate, implying months of preparation before a date days away, or treating a past peak as ongoing without a supplied active window is a timing defect even when the date itself is traceable. Past context must be identified as past; practical preparation must not invent an astrological lead-in.",

...
import {
  ownerApprovedNatalHouseMechanism,
  ownerApprovedSolarReturnOverlay
} from "./ask-tldr-governed-authority-snapshots.js";
import {
  ownerApprovedEclipseHousePlacement,
  ownerApprovedTwelfthHouseProfection
} from "./ask-tldr-governed-technique-authorities.js";
...
type GovernedMeaningSourceKind =
  | "knowledge_index"
  | "owner_approved_cms_snapshot"
  | "owner_approved_internal_mechanism"
  | "owner_approved_manifestation_set"
  | "owner_approved_profection_doctrine";
...
export function resolveAskTldrGovernedFactor(candidate: RankedCalculatedEvidence): AskTldrGovernedFactor {
  ...
  if (status !== "full") {
    const snapshotMeaning = ownerApprovedNatalHouseMechanism(candidate, explicit.unresolvedKnowledgeIds)
      ?? ownerApprovedSolarReturnOverlay(candidate, explicit.unresolvedKnowledgeIds)
      ?? ownerApprovedEclipseHousePlacement(candidate, explicit.unresolvedKnowledgeIds)
      ?? ownerApprovedTwelfthHouseProfection(candidate, explicit.unresolvedKnowledgeIds)
      ?? cmsTransitGovernedMeaning(candidate, surface, explicit.unresolvedKnowledgeIds);
    if (snapshotMeaning) return { ...candidate, governedMeaning: snapshotMeaning };
  }
